...
function projectInspection(inspection: Awaited<ReturnType<FrontdoorOrchestrator['inspectRun']>>): Record<string, unknown> {
  return {
    ...
    plan: { planId: inspection.plan.planId, requestId: inspection.plan.requestId, version: inspection.plan.version, aggregationPolicy: inspection.plan.aggregationPolicy, nodeReviewPolicy: inspection.plan.nodeReviewPolicy, planHash: inspection.plan.planHash, nodes: inspection.plan.nodes.map((node) => ({ nodeId: node.nodeId, objective: boundedText(node.objective, 1000), role: node.role, adapterId: node.adapterId, participantId: node.participantAssignment?.participantId, participantRole: node.participantAssignment?.role, assignmentId: node.participantAssignment?.assignmentId, dependsOn: node.dependsOn, depth: node.depth })) },
    // `compatibility` is carried through: a window AI reading this projection has to be able to see
    // that a Gate was passed on a backward-compatibility route, not just that it was passed.
    decisions: inspection.decisions.map((decision) => ({ decisionId: decision.decisionId, gate: decision.gate, decision: decision.decision, targetHash: decision.targetHash, approvedBy: boundedText(decision.approvedBy, 120), decidedAt: decision.decidedAt, compatibility: decision.compatibility })),
    ...
    participantEvidence: inspection.participantEvidence,
    workPlaneArtifact: inspection.workPlaneArtifact,
  ...
  private async callTool(name: string, args: Record<string, unknown>): Promise<McpToolCallResult> {
    switch (name) {
      ...
      case 'adf_frontdoor_get_result': {
        ...
        return toolResult({ runId, state: inspection.run.state, ownerGate: inspection.run.ownerGate, ownerNextAction: inspection.nextAction, aggregate: inspection.aggregate, participantEvidence: inspection.participantEvidence ?? [], results })
      }

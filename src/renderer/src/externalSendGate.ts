 ...
 * without rendering. `local-http` Adapters (Ollama, LM Studio, etc.) require a fresh, passing readiness check in
 * addition to preflight — an Owner-explicit action, never inferred or assumed stale-valid.
 */
export function isSendEnabled(preflight: ExternalPreflight | null, readiness: LocalModelReadiness | null, connection: string | undefined, busy: boolean, inFlight: boolean): boolean {

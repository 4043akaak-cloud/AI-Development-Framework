...
import type { FrontdoorEventType, FrontdoorLedgerEvent, OrchestrationRun } from '../../shared/frontdoorTypes'
...
export function replayFrontdoorRun(events: readonly FrontdoorLedgerEvent[]): OrchestrationRun {
  ...
  for (const event of events.slice((created?.sequence ?? 0) + 1)) {
    ...
    if (event.type === 'frontdoor.approval-bound') run = { ...run, state: 'running', approvalIds: Array.isArray(payload.approvalIds) ? payload.approvalIds as string[] : run.approvalIds }
    ...
    if (event.type === 'frontdoor.run-recovery-needed') run = { ...run, state: 'awaiting-owner' }
    if (event.type === 'frontdoor.run-stopped') run = { ...run, state: 'cancelled' }
    if (event.type === 'frontdoor.run-completed') run = { ...run, state: payload.status as OrchestrationRun['state'], aggregateResultRef: typeof payload.aggregateRef === 'string' ? payload.aggregateRef : run.aggregateResultRef, openQuestionIds: Array.isArray(payload.openQuestionIds) ? payload.openQuestionIds as string[] : run.openQuestionIds }
